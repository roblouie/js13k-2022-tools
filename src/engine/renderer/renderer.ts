...
export class Renderer {
  ...
  render(camera: Camera, scene: Scene, isWireframe = false) {
    ...
    const renderSkybox = (skybox: Skybox) => {
      ...
      gl.drawArrays(isWireframe ? gl.LINES : gl.TRIANGLES, 0, 6);
    }
    ...
    const renderMesh = (mesh: Mesh) => {
      ...
      if (isWireframe) {
        gl.drawArrays(gl.POINTS, 0, mesh.geometry.getAttribute(AttributeLocation.Positions).data.length / 3);
      }
    }
